...
function SidebarContent() {
  ...
  return (
    <RootWrapper sx={{overflow: 'hidden'}}>
      <Box display="flex" alignItems="flex-start" className='mb-2'>
        <Avatar alt={user.name} src={user.avatar} className="mx-2" />
        <Box
          sx={{
            ml: 1.5,
            flex: 1
      ...
      <TabsContainerWrapper>
        ...
        >
          {tabs.map((tab) => (
            <Tab key={tab.value} label={t(`messenger.${tab.label}`)} value={tab.value} />
          ))}
        </Tabs>
      </TabsContainerWrapper>
        ...
        {currentTab === 'archived' && (
          <Box pb={3}>
            ...
            <Typography
              ...
              variant="subtitle2"
            >
              {t('messenger.Hurray! There are no archived chats!')}
            </Typography>
            <Divider
              sx={{
                mt: 3
